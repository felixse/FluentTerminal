...
namespace FluentTerminal.SystemTray
{
    public class LoopbackEnabler
    {
        ...
        internal List<LoopbackEnabler.INET_FIREWALL_APP_CONTAINER> _AppList;
        internal List<LoopbackEnabler.SID_AND_ATTRIBUTES> _AppListConfig;
        public List<AppContainer> Apps = new List<AppContainer>();
        internal IntPtr _pACs;

        public LoopbackEnabler()
        {
            LoadApps();
        }

        public void EnableForApp(string appName)
        {
            var app = Apps.SingleOrDefault(a => a.displayName == appName);
